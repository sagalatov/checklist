...
Пример chmod 766 file

### chown
Команда chown очень позволяет только менять владельца и группу
$ chown пользователь опции /путь/к/файлу
chown root ./dir1 (изменим владельца папки dir1 на root:)
chown root:root ./dir2 (поменять сразу владельца и группу каталога)
chown -R root:root ./dir3 (применить изменения ко всем подкаталогам -R)  

### ssh
Сетевой протокол. Используется для удаленного управления операционными системами и передачи файлов. Ключевая особенность заключается в том, что SSH шифрует трафик, делая подключения безопасными. 
...
7. top/kill
top
top (table of processes) — консольнаякоманда, которая выводит список работающих в системе процессов и информацию о них. По умолчанию она в реальном времени сортирует их по нагрузке на процессор.
...
* COMMAND - имя процесса (bb_monitor.p
