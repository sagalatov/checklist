...
### Интерфейс
Интерфейс это уровень абстракции который говорит что данный класс будет делать, а абстракцией ниже уже находятся методы которые определяют как. (Логика выполнения может меняться)  
