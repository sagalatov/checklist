...
### CSP - Content security policy
Технология позволяющая заспознать и устранить атаки внедрения данных такие как XSS. Может быть настроен с помощью тэга meta.
...
#### SSL 
SSL или его наследник TLS прикладной протакол, более высокого уровня чем TCP/IP. Позволяет осуществлять безопасное HTTP соединение (HTTPS)
...
#### Сертификаты CA
Для аутентификации полльзователей в сети https используются цифровые сертификаты. По сути, сертификаты связывают доменные имена с определенным публичным ключом.

Совместно все выше указанные технологии похволяют установить https соединение.